...
a numerous benefits, which nginx provides, in the official documentation and many articles about
this topic. The most frameworks have an examples of a nginx configuration for running web
application, based on these frameworks. For example, you can see such example in the
...
is [lua-nginx-module](https://github.com/openresty/lua-nginx-module#readme), which makes able to run
Lua scripts directly from nginx site configuration with help of
the [LuaJIT](https://luajit.org/luajit.html) compiler. This module allows doing necessary checks
before pass the request for further processing, modifying original request or redirecting the user
to another URL. The SingleA client for nginx, which is described below, is based on this approach.

## Installation

The first you need install nginx compiled
with [lua-nginx-module](https://github.com/openresty/lua-nginx-module) and LuaJIT 2.1 (or later).
Besides, you need to install the following lua packages that the SingleA nginx Lua client depends
on:
...
main nginx configuration file (`/etc/nginx/nginx.conf`), you need to add the following directives in
the `http` section (use your path to directory with Lua scripts):
...
The `lua_shared_dict` directive defines a dictionary (key-value storage that shared between all
nginx workers), which will be used to cache user tokens that received from the SingleA server. It
will be described in more details below, here it is only should be noted that the dictionary
name `tokens` can be any other else, and you can allocate different amount of memory for the
dictionary (not 1 megabyte as in the example above).

## Configuration

There are 2 ways to configure the SingleA client for nginx: by explicit specifying of parameters
when initialize the client, and by environment variables. The second one is especially useful when
you deploy the client application with help of containers and follow
the [12-Factor App](https://12factor.net/) principles. See an example of explicit configuration
[below](#examples).

!!! note ""

    To access environment variables from Lua in the nginx configuration context, you should use the
    [`env` directive](https://nginx.org/en/docs/ngx_core_module.html#env). It makes it possible to
    use system-wide defined environment variables or define your own, which will be used only by the
    nginx workers.

Each explicit parameter of the client constructor has a paired environment variable, which has an
uppercase name with the prefix `SINGLEA_` (or another specified in the `env_prefix` explicit
parameter). For example, the value of the `client_id` parameter can be specified
via `SINGLEA_CLIENT_ID` environment variable. This can be helpful, if you configure requests
proxying to multiple client applications through one nginx instance.
...
The SingleA client parameters are listed below. In the beginning are listed mandatory parameters (
marked with `*`), which must be specified by any way. Names of parameters will be shown according
...
* `client_id`* — the client identifier that was returned after registration.
* `secret`* — the client secret that was returned after registration.
* `client_base_url` — scheme and domain (with a port, if necessary) of the client application. By
  ...
  [Traefik](https://doc.traefik.io/traefik/), which handles request by https (port 443) and pass
  them to the nginx instance, which works on http (port 80). In this case the `ngx.var.scheme`
  will be equal to `http` that is incorrect.
...
* `signature_md_algorithm` (default: `SHA256`) — the message digest algorithm (hash function) used
  for request signature.
* `request_timeout` (default: `30`) — the timeout for internal requests between the SingleA client
  and the server (user session verification and user token generation).
* `ssl_not_verify` — if any non-empty value is specified, it prevents SSL certificate validation for
  internal requests between the SingleA client and the server. This can be useful in the development
...
??? info "Shared dictionary for cache user tokens"

    Shared dictionary is created by the `lua_shared_dict` nginx directive, which was mentioned above
    in the [Installation](#installation) section. It creates the shared between all nginx workers
    key-value in-memory storage, which size is limited by the specified in the directive. When the
    SingleA client receive the user token, it stores this token in the storage (cache) for the time
    specified in the `Cache-Control: max-age` HTTP header (if it exists in the response with the
    token, otherwise the token is not cached). Until the cache item will expire (or will be flushed,
    see `token_flush_header` parameter description below), the token value will be fetched from the
    cache instead of request from the SingleA server.
...
* `token_header` (default: `Authorization`) — the name of an HTTP header of an original request,
  which should be written by the SingleA client and must contain the user token that received from
...
* `token_prefix` (default: `Bearer `, with a space in the ending) — the prefix, which should be
  added to the token header before of the user token value.
* `token_flush_header` (default: `X-Flush-Token`) — the name of an HTTP header, which used in 2
  cases:
    * You can flush the user token with help of the `flush_token()` client method **after** the
      request processing. This method looks for the flush header in the response, which was
      generated by the client application (see the second example [below](#examples)).
...
* `redirect_uri_query_parameter` (default: `redirect_uri`) — the name of the GET parameter with the
  URI where the user should be redirected after successful operation on the SingleA server side
  (used for user login and logout).
* `login_path`, `logout_path`, `validate_path`, `token_path` (defaults: `/login`, `/logout`,
  `/validate` and `/token`) — relative paths on the SingleA server side of the methods for the user
  login, logout, for the user session validation and for the user token generation.
...
!!! important ""
    ...
    the routes cusomization (through the `config/routes.yaml` and `config/routes/*.yaml` configs).

!!! caution ""

## Usage

The SingleA client for nginx has the following methods.
...
- `token(auth_required=true)` — make an HTTP request to the SingleA server to receive the
  authenticated user token. The decision to check it or not the user session is up to the client
...
Below are few examples of using the SingleA client for nginx.
...
the [Payload Fetcher](../features/payload-fetcher.md) feature, do not forget to exclude such kind of
...
If you need to process `OPTIONS` requests, you can handle such kind of requests directly in lua
code:
