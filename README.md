...
3. Verify (`/verify`) and token (`/token`) controllers configured as public accessible, but actually
...
> You need to customize the `SingleA\Authentication\EventListener\SuccessfulLoginListener::setPayload`
> method to add some real payload data for an authenticated user. If you do not do this, the user
> token payload will not contain any data.
