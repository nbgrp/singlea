...
final class SignatureService implements SignatureServiceInterface
{
    ...
    public function check(Request $request, SignatureConfigInterface $config): void
    {
        ...
        $clientTime = (int) $request->query->get($this->timestampQueryParameter);
        $serverTime = (int) $session->get(KeepMetaCreatedSessionAuthenticationStrategy::INITIAL_META_CREATED, $session->getMetadataBag()->getCreated());
        $serverTime -= $config->getClientClockSkew();

        if ($clientTime > $serverTime || $serverTime - $this->requestTtl > $clientTime) {
